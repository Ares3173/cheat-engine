...
int setVM_CR0(pcpuinfo currentcpuinfo, UINT64 newcr0)
{
  ...
  else if ((oldcr0 & (1 << 31)) && (!(newcr0 & (1 << 31))))
  {
    ...
    if (shouldInvalidate)
    {
      ...
      else
      {
        ...
        if (has_VPID_INVVPIDSingleContextRetainingGlobals)
          type = 3;
        else if (has_VPID_INVVPIDSingleContext)
          type = 2;
        else
          type = 1; // all
...
int handleSingleStep(pcpuinfo currentcpuinfo, VMRegisters *vmregisters, FXSAVE64 *fxsave)
{
  ...
  while (currentcpuinfo->singleStepping.ReasonsPos)
  {
    ...
    {
    case SSR_HANDLEWATCH:
      r = ept_handleWatchEventAfterStep(currentcpuinfo, currentcpuinfo->singleStepping.Reasons[i].ID);
      break;
    case SSR_HANDLECLOAK:
      r = ept_handleCloakEventAfterStep(currentcpuinfo, (PCloakedPageData)currentcpuinfo->singleStepping.Reasons[i].Data);
      break;
    case SSR_HANDLESOFTWAREBREAKPOINT:
      r = ept_handleSoftwareBreakpointAfterStep(currentcpuinfo, currentcpuinfo->singleStepping.Reasons[i].ID);
      break;
    // todo: case SSR_STEPTILLINTERUPTABLE: return singleStepTillInteruptable
    case SSR_STEPANDBREAK:
      r = ept_handleStepAndBreak(currentcpuinfo, vmregisters, fxsave, currentcpuinfo->singleStepping.Reasons[i].ID);
      break;
    default:
    {
      nosendchar[getAPICID()] = 0;
